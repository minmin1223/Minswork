...
parser.add_argument('--video_detect', default=False, type=str, help='using video detection')
...
dataset = COCODetection(cfg, mode='train')
 # AdamW              
optimizer = optim.SGD(net.parameters(), lr=cfg.lr, weight_decay=5e-4)
...
if cfg.cuda:
    ...
    if args.resume==True:
        ...
        cfg = get_config(args, mode='train')
data_loader = data.DataLoader(dataset, cfg.train_bs, num_workers=0, shuffle=False,
...
print('epochs: ', total_num_epochs,"start_time",datetime.datetime.now()) # add
...
try:  # try-except can shut down all processes after Ctrl + C.
    ...
    for epoch in range(0, total_num_epochs, 1):
        ...
        for images, targets, masks in tqdm(data_loader):
            ...
            with timer.counter('for+loss'):
                ...
                #print("loss_c",loss_c,"loss_b",loss_b,"loss_m",loss_m,"loss_s",loss_s)
            ...
            if step % int(args.show_every_iters) == 0 and step != start_step:
                ...
                (f'epoch: {epoch} | step: {step} | lr: {cur_lr:.2e} | ls: {loss:.3f} | l_class: {l_c:.3f} | l_box: {l_b:.3f} | '
        ...
        if epoch % int(args.plot_every_iters) == 0 :
            ...
            if args.plot == True:
                ...
                plt.grid()
                #plt.show()
                plt.savefig(f'results/last_{cfg_name}_{step}.png')
            ...
            net.train()
            #save_latest(net if cfg.cuda else net, cfg_name, step)#add
    ...
    import glob
    weight = glob.glob('weights/best*') #best*
