...
def evaluation(net, cfg, step=None):
    ...
    for i, (img, gt, gt_masks, (img_h, img_w), (img_origin, img_name)) in enumerate(data_loader):
        ...
        if i > 0:
            ...
            if cfg.measure == True:
                print(f'\Testing: {bar_str} {i + 1}/{ds}, fps: {fps:.2f} | total fps: {t_fps:.2f} | '
        ...
        if cfg.measure == True:
            ...
            if len(mae['img']) > 0 and len(mse['img']) > 0:
                ...
                if mMAE < 0.425 and mMSE < 0.378:
                    print('stop')
        result_df = pd.DataFrame.from_dict(result) #originalno
        result_df.to_csv(cfg.env_path+'/results/result_df.csv', index=False) #originalno
