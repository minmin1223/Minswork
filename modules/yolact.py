...
from modules.upanets_v2 import UPANets
from modules.resnet import ResNet
from modules.swin_transformer import SwinTransformer
from modules.rtmdet import CSPDarknet, PAFPN
from modules.resnet_panel import ResNet_Panel
    
class PredictionModule(nn.Module):
    def __init__(self, cfg, coef_dim=32, in_channel=256):
        super().__init__()
        self.cfg = cfg
        self.num_classes = cfg.num_classes
        self.coef_dim = coef_dim

        self.upfeature = nn.Sequential(nn.Conv2d(in_channel, in_channel, kernel_size=3, padding=1),
...
class ProtoNet(nn.Module):
    def __init__(self, coef_dim=32, in_channel=256):
        super().__init__()
        ...
        self.proto2 = nn.Sequential(
                                    nn.Conv2d(in_channel, in_channel, kernel_size=3, stride=1, padding=1),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(in_channel, coef_dim, kernel_size=1, stride=1),
                                    nn.ReLU(inplace=True)
                                    )
...
class FPN(nn.Module):
    def __init__(self, in_channels, out_channels = 256):
        ...
        self.downsample_layers = nn.ModuleList([nn.Sequential(nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, stride=2),
                                                              nn.ReLU(inplace=True)),
                                                nn.Sequential(nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, stride=2),
                                                              nn.ReLU(inplace=True))])
    ...
    def forward(self, backbone_outs):
        ...
        p5_1 = self.lat_layers[2](backbone_outs[2])# + p6_upsample
        ...
        p4_1 = self.lat_layers[1](backbone_outs[1]) + p5_upsample
        p4_upsample = self.upsample_module[0](p4_1)
        p4_downsample = self.downsample_module[0](p4_1)

        p3_1 = self.lat_layers[0](backbone_outs[0]) + p4_upsample
        p3_downsample = self.downsample_module[0](p3_1)


        p4_2 = p4_1 + p3_downsample
        p5_2 = p5_1 + p4_downsample

        p5 = self.pred_layers[2](p5_2)
        p4 = self.pred_layers[1](p4_2)
        p3 = self.pred_layers[0](p3_1)
        #p6 p7 for proto
        p6 = self.downsample_layers[0](p5_1)
...
class Yolact(nn.Module):
    def __init__(self, cfg):
        ...
        self.prediction_layers = PredictionModule(cfg)
        self.semantic_seg_conv = nn.Conv2d(256, cfg.num_classes - 1, kernel_size=1)
                           
        if cfg.backbone == 'upanet16':
            self.backbone = UPANets(16, 1)
            self.fpn = FPN(in_channels=(64, 128, 256), out_channels=256)
                      
        if cfg.backbone == 'upanet64':
            self.backbone = UPANets(64, 1)
            self.fpn = FPN(in_channels=(256, 512, 1024))
            
        if cfg.backbone == 'upanet80':
            self.backbone = UPANets(80, 1)
            self.fpn = FPN(in_channels=(320, 640, 1280))
            #self.fpn = PAFPN(in_channels=(320, 640, 1280)) 參數太多
        
        if cfg.backbone == 'upanet96':
            self.backbone = UPANets(96, 1)
            self.fpn = FPN(in_channels=(384, 768, 1536))
            
        if cfg.backbone == 'resnet50':
            self.backbone = ResNet(layers=(3, 4, 6, 3))
            self.fpn = FPN(in_channels=(512, 1024, 2048))
        
        if cfg.backbone == 'resnet50-panel':
            self.backbone = ResNet_Panel(layers=(3, 4, 6, 3))
            self.fpn = FPN(in_channels=(512, 1024, 2048))
        ...
        if cfg.backbone == 'swimtransformer':
    ...
    def forward(self, img, box_classes=None, masks_gt=None, img_size=None):
        ...
        return class_pred, box_pred, coef_pred, proto_out, None
    ...
    def compute_loss(self, class_p, box_p, coef_p, proto_p, seg_p, box_class, mask_gt, fpn_features):
        ...
        for i in range(batch_size):
            ...
            all_offsets[i], conf_gt[i], anchor_max_gt[i], anchor_max_i[i], mask_gt[i], class_gt[i] = match(self.cfg,
                                                                                  box_gt,
                                                                                  self.anchors,
                                                                                  class_gt[i],
                                                                                  mask_gt[i],
        ...
        if pos_bool.sum() > 0:
            ...
            loss_s = self.semantic_seg_loss(seg_p, mask_gt, class_gt)
             ...
             loss_c, loss_b, loss_m, loss_s = torch.tensor(0.), torch.tensor(0.), torch.tensor(0.), torch.tensor(0.)
    ...
    def lincomb_mask_loss(self, pos_bool, anchor_max_i, coef_p, proto_p, mask_gt, anchor_max_gt):
        ...
        for i in range(coef_p.size(0)):
            ...
            if old_num_pos > self.cfg.masks_to_train:
                #print("old_num_pos > self.cfg.masks_to_train")
            ...
            mask_p = torch.where(torch.isnan(mask_p), torch.zeros_like(mask_p), mask_p) 
